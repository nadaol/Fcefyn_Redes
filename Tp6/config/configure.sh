...
sudo docker exec -ti --privileged h1 ip route add default via 20.0.0.2
sudo docker exec -ti --privileged h1 ip -6 route del default
sudo docker exec -ti --privileged h1 ip -6 route add default via 20::2

#H3
sudo docker exec -ti --privileged h2 ip route del default
sudo docker exec -ti --privileged h2 ip route add default via 20.0.0.2
sudo docker exec -ti --privileged h2 ip -6 route del default
sudo docker exec -ti --privileged h2 ip -6 route add default via 20::2
